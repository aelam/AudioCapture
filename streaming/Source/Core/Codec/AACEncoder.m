...
- (void) setupEncoderFromSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    outAudioStreamBasicDescription.mChannelsPerFrame = 1;//inAudioStreamBasicDescription.mChannelsPerFrame; // The number of channels in each frame of audio data. This value must be nonzero.
    ...
    if (status != 0) {
        NSLog(@"[converter]setup converter: %d", (int)status);
    } else {
        NSLog(@"[converter]setup converter: success! %d", (int)status);
    }
}
